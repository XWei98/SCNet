...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        #x.shape
        #torch.Size([16, 784, 64])
        #self.po
        #torch.Size([1, 196, 64])

        #print("x.shape   ",x.shape)
        #print("self.po   ",self.position_embeddings.shape)
...
class VisionTransformer(nn.Module):  # Transformer-branch
    ...
    def forward(self, x, skip_x, text, reconstruct=False):
        ...
        if not reconstruct:
            #print("x", x.shape) x torch.Size([10, 64, 448, 448])
            x = self.embeddings(x)
            #embeddings torch.Size([10, 196, 64])
            #print("embeddings", x.shape)
            if self.dim == 64:
                #print("self.CTBN3(text).shape", self.CTBN3(text).shape)torch.Size([10, 196, 64])
                #print("sx", x.shape)sx torch.Size([10, 196, 64])
